...
from fr_model import Model, RiverObject, Frog, Car, Log
...
def test_car():
    ...
    speed = 10
    carnum = 1
    car1 = Car(x, y, carnum, Direction.LEFT, speed)
    assert(car1.get_position() == (x,y))
    car1.move()
    assert(car1.get_position() == (x-speed, y))

    car2 = Car(x, y, carnum, Direction.RIGHT, speed)
    assert(car2.get_position() == (x,y))
    car2.move()
    assert(car2.get_position() == (x+speed, y))
    
    x1 = -CANVAS_WIDTH//2 + 1.5 * speed
    car3 = Car(x1, y,carnum,Direction.LEFT,speed)
    assert(car3.get_position() == (x1, y))
    car3.move()
    assert(car3.get_position() == (x1-speed, y))
    car3.move()
    assert(car3.get_position() == (CANVAS_WIDTH, y))
    
    car4 = Car(x, y, carnum, Direction.LEFT, speed)
